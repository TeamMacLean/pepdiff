...
**Note:** Bayes factors are fundamentally different from p-values - they quantify evidence rather than surprise. The current implementation converts BF to approximate p-values for compatibility, but this is a simplification. Interpret BF results with care.

The Bayes factor approach asks a different question. Classical tests ask: "If there's no effect, how surprising is this data?" (p-value). Bayes factors ask: "How much more likely is the data under the hypothesis of an effect vs no effect?"

**Interpreting BF:**
- BF = 1: Data equally consistent with effect or no effect
- BF = 3: Data 3× more likely under effect hypothesis (moderate evidence)
- BF = 10: Strong evidence for effect
- BF = 0.1: Strong evidence for *no* effect (this is informative - p-values can't tell you this)
...
All tests report both raw p-values and FDR-adjusted values. **Always use the `fdr` column for significance decisions.**
