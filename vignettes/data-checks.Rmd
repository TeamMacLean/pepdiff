...
We would usually like to know how the values of quantification are distributed, as it helps us determine a modelling strategy for statistical tests later.. We can do that with `plot_quant_distributions()`.

```{r}
plot_quant_distributions(raw, log = FALSE)
```
...
```{r}
plot_quant_distributions(raw, log = TRUE, base = 2)
```
...
Log transformations can make the data more normally distributed, this could be useful for tests later, too

```{r}
plot_norm_qq(raw, log=TRUE, base = 2)
```

That looks better. We can also go further and examine a range of distributions, for that we'll use the `fitdistrplus` package. We can convert our input raw data to a suitable format with `to_vec()`
...
These plots are fun to interpret! They measure skewness and kurtosis which are metrics of shape in distributions. These are plotted along with examples from known distributions. Bootstrap samples to estimate variability are also taken and plotted.

The examples that most closely fit our observed data give us an idea of what our distribution is. The log-normal and gamma distributions have lines, not points as they fulfill a range of skew and kurtosis values. 

We can see that our data fit the gamma distribution most closely, which is useful for downstream analysis.
