...
```{r}
td <- combine_tech_reps(d)
```

## Look at distribution of quantifications

```{r}
plot_quant_distributions(td)
plot_quant_distributions(td, log = TRUE)
norm_qqplot(td)
norm_qqplot(td, log = TRUE)
```
...
```{r}
r <- calc_de(td, control = 'Col-0', c_seconds = '0', treatment = 'Col-0', t_seconds = '150', iters = 10000)
```
